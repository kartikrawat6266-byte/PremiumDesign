...
def owner_keyboard():
    return InlineKeyboardMarkup([
        ...
        [
            InlineKeyboardButton("❌ Declined", callback_data="owner_declined"),
...
async def owner_activity(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, user in list(data.items())[-15:]:
...
async def owner_declined(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = (
        "❌ Declined Payments Panel\n\n"
        "Cancelled payments automatically appear in owner logs."
    )
...
async def owner_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid, user in list(data.items())[:25]:
